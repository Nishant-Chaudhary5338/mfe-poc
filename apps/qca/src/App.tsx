      ...
      <div className="flex h-screen overflow-hidden font-sans">
        <nav className="flex w-[220px] shrink-0 flex-col overflow-hidden" style={{ background: COLOR_DARK }}>
          <div className="border-b border-white/[0.08] px-4 py-5">
            <div className="flex items-center gap-2.5">
              <div className="flex size-9 shrink-0 items-center justify-center rounded-[9px] bg-white/[0.12] font-[Sora] text-[10px] font-bold tracking-widest text-white">QCA</div>
              <div>
                <p className="font-[Sora] text-xs font-bold leading-tight text-white">QC Automation</p>
            ...
            {navItems.map(item => (
              <NavLink key={item.path} to={item.path} end={item.end}
                className={({ isActive }) =>
                  ...
                  (isActive ? 'bg-white/[0.09] font-semibold text-white' : 'border-l-transparent font-normal text-white/50 hover:bg-white/[0.05] hover:text-white/80')
                }
        ...
        <div className="flex flex-1 flex-col overflow-hidden">
          <div className="flex h-[52px] shrink-0 items-center border-b border-slate-200 bg-white px-6">
            <span className="text-[13px] font-medium text-slate-500">QC Automation · Quality Pipeline</span>
          </div>
