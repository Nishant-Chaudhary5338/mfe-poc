...
function ChunkCard({ entry, color, bg, border, strikethrough }: { entry: DiffEntry; color: string; bg: string; border: string; strikethrough?: boolean }) {
  ...
  return (
    <div style={{ background: bg, borderRadius: 8, border: `1px solid ${border}`, borderLeft: `3px solid ${color}`, padding: '10px 14px', display: 'flex', flexDirection: 'column', gap: 3 }}>
      <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
        {tagStyle && (
          <span style={{ fontSize: 10, fontWeight: 700, letterSpacing: '0.05em', fontFamily: 'var(--font-mono)', padding: '2px 6px', borderRadius: 4, background: tagStyle.bg, color: tagStyle.text, border: `1px solid ${tagStyle.border}` }}>{entry.tag}</span>
        )}
        <span style={{ fontFamily: 'var(--font-mono)', fontSize: 12, fontWeight: 600, color: strikethrough ? 'var(--muted)' : 'var(--text)', textDecoration: strikethrough ? 'line-through' : 'none', flex: 1, wordBreak: 'break-all' }}>{entry.name}</span>
      ...
      {entry.oldName && <div style={{ fontSize: 10, color: 'var(--muted)', fontFamily: 'var(--font-mono)', marginTop: 1 }}>was: {entry.oldName}</div>}
      {entry.source && <div style={{ fontSize: 11, color: 'var(--muted)', fontFamily: 'var(--font-mono)' }}>{entry.source}</div>}
      {entry.description && <div style={{ fontSize: 11, color: 'var(--muted)', lineHeight: 1.5 }}>{entry.description}</div>}
    </div>
  );
}

function DiffSection({ title, entries, type, strikethrough }: { title: string; entries: DiffEntry[]; type: keyof typeof DIFF_COLORS; strikethrough?: boolean }) {
    ...
    <div style={{ background: 'var(--card)', borderRadius: 12, border: '1px solid var(--border)', overflow: 'hidden', boxShadow: 'var(--shadow-sm)' }}>
      ...
      <div style={{ padding: '16px 20px', maxHeight: 480, overflowY: 'auto' }}>
        ...
        {diff.unchanged.length > 0 && (
          <div>
            <button onClick={() => setShowUnchanged(v => !v)} style={{ display: 'flex', alignItems: 'center', gap: 8, background: 'var(--surface)', border: '1px solid var(--border)', borderRadius: 6, padding: '7px 12px', cursor: 'pointer', color: 'var(--muted)', fontSize: 12, fontWeight: 600, fontFamily: 'var(--font-body)', marginBottom: showUnchanged ? 8 : 0, width: '100%' }}>
              <span style={{ transform: showUnchanged ? 'rotate(90deg)' : 'none', display: 'inline-block', transition: 'transform 0.15s', fontSize: 10 }}>▶</span>
              {diff.unchanged.length} unchanged — no re-testing required
    ...
    <div style={{ background: 'var(--card)', borderRadius: 12, border: '1px solid var(--border)', overflow: 'hidden', boxShadow: 'var(--shadow-sm)' }}>
      ...
      <div style={{ padding: '12px 16px', display: 'flex', flexDirection: 'column', gap: 6 }}>
        {summaries.map(s => {
          ...
          return (
            <div key={s.id} style={{ display: 'flex', alignItems: 'center', gap: 12, padding: '11px 14px', borderRadius: 9, background: isNew ? DIFF_COLORS.added.bg : 'var(--surface)', border: `1px solid ${isNew ? DIFF_COLORS.added.border : isClean ? '#BBF7D0' : 'var(--border)'}` }}>
              <span style={{ fontFamily: 'var(--font-mono)', fontSize: 11, fontWeight: 700, color: 'var(--text)', width: 36, padding: '2px 6px', borderRadius: 4, background: 'var(--border)', textAlign: 'center' }}>{s.id.toUpperCase()}</span>
...
export default function BuildCompare() {
  ...
  const canBuild = user?.role !== 'viewer';
  ...
  function refreshApps() {
    fetch('/api/apps').then(r => r.json()).then((apps: { id: string; label: string }[]) => setRegisteredApps(apps)).catch(() => {});
  }
  ...
  async function handleBuild() {
    ...
    setDiff(null);
    setLog('');
    toast.info(`Building ${appId === 'all' ? 'all plugins' : appId.toUpperCase()}…`);
      ...
      for (const chunk of lines) {
        ...
        try {
          ...
          if (evt.type === 'stdout' || evt.type === 'stderr') {
            setLog(l => l + (evt.line ?? '').replace(/\r/g, ''));
          } else if (evt.type === 'done' || evt.type === 'exit') {
            setBuildDone(true);
            setBuilding(false);
            setSnapshotDone(true);
            toast.success('Build complete', 'Snapshot saved — ready to compare');
          }
        } catch {}
      }
    }
  ...
  async function handleCompare() {
    if (appId === 'all') {
      ...
      const results = await Promise.all(apps.map(id => fetch(`/api/compare?appId=${id}`).then(r => r.json())));
      ...
      if (data.error) { setDiff(null); toast.error('Compare failed', data.error); }
      else setDiff(data);
    }
  }
  ...
  /* ── Step indicator ─────────────────────────────── */
  const steps = [
    { n: 1, label: 'Select plugin', done: !!appId },
    { n: 2, label: 'Build',         done: buildDone },
    { n: 3, label: 'Compare',       done: !!(diff || multiDiff) },
  ];

  return (
    <div style={{ maxWidth: 1100 }}>
      <div style={{ marginBottom: 20 }}>
        <h1 style={{ fontFamily: 'var(--font-head)', fontSize: 22, fontWeight: 800, color: 'var(--text)', margin: 0, letterSpacing: '-0.02em' }}>Build & Compare</h1>
        <p style={{ color: 'var(--muted)', fontSize: 13, marginTop: 3 }}>Build plugins, snapshot dist/ chunks, and diff changes between builds</p>
      </div>

      {/* Workflow stepper */}
      <div style={{ display: 'flex', alignItems: 'center', gap: 0, marginBottom: 24, background: 'var(--card)', borderRadius: 12, border: '1px solid var(--border)', padding: '14px 20px', boxShadow: 'var(--shadow-xs)' }}>
        {steps.map((s, i) => (
          <div key={s.n} style={{ display: 'flex', alignItems: 'center', gap: 0, flex: i < steps.length - 1 ? 1 : 'none' }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
              <div style={{
                width: 28, height: 28, borderRadius: '50%', flexShrink: 0,
                display: 'flex', alignItems: 'center', justifyContent: 'center',
                fontSize: 11, fontWeight: 800,
                background: s.done ? 'var(--green)' : i === 0 ? 'var(--signal)' : 'var(--border)',
                color: (s.done || i === 0) ? '#fff' : 'var(--muted)',
                transition: 'background 300ms',
              }}>
                {s.done ? '✓' : s.n}
              </div>
              <div>
                <div style={{ fontSize: 12, fontWeight: 600, color: s.done ? 'var(--green)' : i === 0 ? 'var(--text)' : 'var(--muted)' }}>{s.label}</div>
              </div>
            </div>
            {i < steps.length - 1 && (
              <div style={{ flex: 1, height: 1, background: s.done ? '#BBF7D0' : 'var(--border)', margin: '0 14px', transition: 'background 300ms' }} />
            )}
          </div>
        ))}
      </div>
      ...
      {!canBuild && (
        <div style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '12px 16px', borderRadius: 10, background: '#FFFBEB', border: '1px solid #FDE68A', marginBottom: 20 }}>
          <span style={{ fontSize: 16 }}>⚠️</span>
          <div>
            <div style={{ fontSize: 13, fontWeight: 700, color: '#92400E' }}>Read-only access</div>
            <div style={{ fontSize: 12, color: '#B45309', marginTop: 1 }}>You can view diff results but cannot trigger builds. Requires editor, ops, or admin role.</div>
          </div>
        </div>
      )}
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '280px 1fr', gap: 20 }}>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
          ...
          {/* Plugin selector */}
          <div style={{ background: 'var(--card)', borderRadius: 12, padding: 16, border: '1px solid var(--border)', boxShadow: 'var(--shadow-xs)' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 10 }}>
              <span style={{ fontSize: 11, fontWeight: 700, color: 'var(--muted)', textTransform: 'uppercase', letterSpacing: '0.07em' }}>Plugin</span>
          ...
          {/* Actions */}
          <div style={{ background: 'var(--card)', borderRadius: 12, padding: 16, border: '1px solid var(--border)', boxShadow: 'var(--shadow-xs)', display: 'flex', flexDirection: 'column', gap: 8 }}>
            <PermissionGate roles={['admin', 'ops', 'editor']} mode="disable" style={{ display: 'block' }}>
              <button onClick={handleBuild} disabled={building} style={{
                ...
                {building ? 'Building…' : `Build ${appId === 'all' ? 'All' : appId.toUpperCase()}`}
              </button>
            </PermissionGate>

            {snapshotDone && (
              <div style={{ fontSize: 11, color: '#059669', padding: '6px 10px', borderRadius: 7, border: '1px solid #BBF7D0', background: '#F0FDF4', display: 'flex', alignItems: 'center', gap: 6 }}>
                <span>✓</span> Snapshot saved
              </div>
            )}
          ...
          <div style={{ background: 'var(--signal-soft)', borderRadius: 10, padding: 14, border: '1px solid #C7D2FE' }}>
            <div style={{ fontSize: 10, fontWeight: 700, color: 'var(--signal)', marginBottom: 8, textTransform: 'uppercase', letterSpacing: '0.07em' }}>How it works</div>
            {[
              'Build → baseline snapshot saved',
              'Modify a route or add code',
              'Rebuild → Compare to see diff',
              'Shell reloads in ~5 seconds',
            ].map((step, i) => (
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
          ...
          <div style={{ borderRadius: 12, border: '1px solid #1E293B', overflow: 'hidden', boxShadow: 'var(--shadow-md)' }}>
            <div style={{ padding: '10px 14px', borderBottom: '1px solid #1E293B', background: '#0F172A', display: 'flex', alignItems: 'center', gap: 8 }}>
              ...
              <button
                ...
                style={{ marginLeft: building || buildDone ? 8 : 'auto', fontSize: 10, padding: '2px 7px', borderRadius: 4, border: `1px solid ${autoScroll ? '#10B981' : '#475569'}`, background: 'transparent', color: autoScroll ? '#10B981' : '#475569', cursor: 'pointer', fontFamily: 'var(--font-mono)' }}
              >
                {autoScroll ? '↓ auto' : '↓ off'}
              </button>
            </div>
